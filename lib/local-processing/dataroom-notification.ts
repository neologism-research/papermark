...
/**
 * Send notifications to dataroom viewers about a new document
 * Local replacement for the Trigger.dev task
 */
export async function sendDataroomChangeNotification(
  payload: NotificationPayload,
) {
  console.log("Processing dataroom notification:", {
    dataroomId: payload.dataroomId,
    documentId: payload.dataroomDocumentId,
  });
  ...
  if (!viewers || viewers.length === 0) {
    console.log("No verified viewers found for this dataroom", {
      dataroomId: payload.dataroomId,
    });
    return;
  }
  ...
  console.log("Processed viewer links", {
  ...
  for (const viewer of viewersWithLinks) {
    try {
      ...
      if (!response.ok) {
        console.error("Failed to send dataroom notification", {
          viewerId: viewer.id,
          dataroomId: payload.dataroomId,
